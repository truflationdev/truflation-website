...
<template>
   <div class="flex flex-row justify-center flex-wrap lg:flex-nowrap gap-8 container mx-auto mt-5">
        <div class="flex flex-col w-full bg-gray-100 py-4 rounded-lg items-center gap-5">
            <div class="flex flex-row gap-3">
               <p class=" text-lg font-semibold">{{ main?.getByCategoryType(selectedCategory)?.categoryType }} Drivers</p>
            </div>
            <div class="flex flex-col w-full gap-2">
              <div class="py-1 px-3 text-gray-600 text-sm font-light" v-for="driver in main?.getByCategoryType(selectedCategory).categoryDrivers">
                <div class=" flex flex-row justify-between" :class="{'driver-selected': selectedCategoryDriver === driver.title}"><button v-on:click="select($event)" :id="driver.title"><p>{{ driver.title}}</p></button> <p>{{ driver.latestValue }}</p></div>
              </div>
            </div>
        </div>
        <div class="flex w-full mx-8 lg:h-full flex-col ml-3 bg-slate-100 p-5 gap-4 h-fit rounded">
          ...
          <Line v-if="selectedCategoryDriver !== 'unknown' && main?.getByCategoryType(selectedCategory).categoryDrivers.find((driver) => driver.title === selectedCategoryDriver)" id="category-chart-id" :options="options" :data="main?.getCategoryDrivers(selectedCategory, selectedCategoryDriver).data"/>
        </div>
