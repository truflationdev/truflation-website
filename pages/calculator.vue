...
<template>
  ...
  <div class="flex relative flex-col z-0 mx-auto container mt-[77px]">
    <div class="flex flex-col w-full items-center px-4 mb-48 lg:hidden">
      ...
      <div class="flex-col my-8 gap-6 flex pr-4">
        <button @click="Calculate()" class="btn px-12">Calculate</button>

        <p class="text-red-700 font-semibold" v-if="error">
          * Error calculating your Inflation Data
        </p>
      </div>
          ...
          <div class="flex-col col-span-1 md:col-span-2 gap-4 flex">
            <button @click="Calculate()" class="btn w-2/3">
              ...
              <svg
                v-if="loading"
                aria-hidden="true"
                class="h-5 w-5 text-blue-600 animate-spin fill-white"
                viewBox="0 0 100 101"
                fill="none"
